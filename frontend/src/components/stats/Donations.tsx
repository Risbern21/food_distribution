  ...
  const fetchMyDonations = async () => {
    try {
      const response = await api.get(
        `/distributions/all_donated/${user?.user_id}`
      );
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-foreground">Your Donations</h1>
          <p className="text-muted-foreground">View your food donations</p>
        </div>
      </div>
